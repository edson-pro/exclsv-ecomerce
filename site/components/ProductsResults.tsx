...
export default function ProductsResults({
  ...
  return (
    <div className="grid md:grid-cols-1 md:gap-0 grid-cols-4 gap-7">
      <div
        className={`${
          showFilters
            ? "md:fixed md:bg-white md:z-[100] md:w-full md:top-0 md:left-0"
            : "md:hidden"
        }`}
      >
        <div className="md:flex hidden border-b border-gray-300 items-center shadow-md justify-between px-4 py-4 border-b border-gray-200 border-opacity-50">
          <div className="flex w-full justify-between items-center">
            ...
            <a
              ...
              className="p-[6px] bg-green-100 cursor-pointer rounded-full text-green-600"
            >
              <X size={13} />
            </a>
          </div>
        </div>
        <div className="md:h-[95vh] md:pb-10 md:pb-[70px] md:overflow-scroll ">
          <Filter
            status={status}
            brands={data?.extra?.brands}
            categories={categories}
          />
        </div>
        <div className="fixed md:block hidden bottom-0 w-full px-3 py-3 bg-white">
          <Button>Apply Filters</Button>
        </div>
      </div>
      <div className={`${isFetching && "p-4 "} col-span-3 relative`}>
        {isFetching && (
          <div className="w-full h-full absolute overflow-hidden top-0 left-0 z-50 rounded-md">
            <div className="flex animate-pulse w-full h-full justify-center bg-white opacity-80 "></div>
            <div className="w-full top-[10%] left-0 absolute  h-full flex justify-center ">
              {/* <Loader primary /> */}
            </div>
          </div>
        )}
        ...
        {status === "success" && data?.data?.length === 0 && <NoRes />}
  ...
  return (
    <div>
      <NoContent
        ...
        action={{
          ...
          onClick: () => {
            router.push({
              ...
              query: router.asPath.split("?")[0].split("/").includes("search")
                ? {
                    q: router.query.q,
                  }
                : {},
        ...
        subTitle="No results found for the query you made ,please chack your typos."
      />
    </div>
  );
}

function SearchIconEmpty() {
  return (
    <svg
      height={80}
      widths={80}
      className="stroke-current fill-current text-gray-700 "
      id="Capa_1"
      viewBox="0 0 512 512"
      width="100"
